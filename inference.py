...
================================
Runs an LLM agent against all 8 tasks in the CodeReview-Env sandbox.
...
You have 8 typed tools. Choose exactly ONE per turn by outputting strict JSON only.
...
TOOL 6: restart_service — run app_init.py to apply changes (tasks 7 & 8 only)
{"action_type": "restart_service"}

── TERMINAL ─────────────────────────────────────────────────────────────────────
TOOL 7: submit_fix — submit when ALL tests pass; include root cause
{"action_type": "submit_fix", "root_cause": "TIMEOUT was str not int; migration sort was lexicographic."}
...
4. For tasks 7-8: fix code first, then call restart_service, then run_tests.
